...
def import_common_apis():
    ...
    try:
        ...
        print "      Make sure your SDE is built for %s target" % (TARGET)
        sys.exit(1)
...
def setdev(device):
    ...
    if PROG == None:
        sys.ps1="PD-Fixed[%d]>>> " % dev
    elif PROG == PREFIX:
        sys.ps1="PD(%s)[%d]>>> " % (str(PROG), dev)
    else:
...
def parse_args():
    ...
    if PROG != None:
        ...
        if args.prefix != None:
            print "WARNING: No P4 program is specified, so prefix is ignored"
            PREFIX = None
